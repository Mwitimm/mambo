...
let checkIn = new Date(customer_details.checkInDate)
let checkOut = new Date(customer_details.checkOutDate)
const timeDiff = checkOut.getTime() - checkIn.getTime();
let days= Math.ceil(timeDiff / (1000 * 60 * 60 * 24));
let date_to_checkIn = customer_details.checkInDate
let date_to_checkOut = customer_details.checkOutDate
const selectedHouse = houses.find(house => house.type === customer_details.houseType);


let daily_amount = selectedHouse.price
let total_amount =  days === 0 ? selectedHouse.price : days * daily_amount;



  console.log("From ",date_to_checkIn,"To ",date_to_checkOut,"With a daily price of ",daily_amount,"We will charge you ",total_amount)
          ...
          <div class="card position-sticky top-0">
              <div class="p-3 bg-light bg-opacity-10">
                  ...
                  <div class="d-flex justify-content-between mb-4 small">
                      <span>Hi {customer_details.name}, Thank you for choosing us</span>
                  </div>
                  <div class="d-flex justify-content-between mb-4 small">
                      <span>We will charge you Kshs {total_amount}</span>
                  </div>
                  <h6 class="card-title mb-3">Billing Summary</h6>
                  <div class="d-flex justify-content-between mb-1 small">
                      <span>Daily price for {selectedHouse.type} house is Kshs {daily_amount}</span>
                  </div>
                  <div class="d-flex justify-content-between mb-1 small">
                      <span>You will be staying from {date_to_checkIn} to {date_to_checkOut}. Enjoy your stay</span>
                  </div>
                  ...
                  <button class="button" onclick="initiateMpesaStkPush()">Pay Khs {total_amount}</button>
              </div>
          </div>
      </div>
  </div>
